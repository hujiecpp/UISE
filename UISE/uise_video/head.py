...
class MultiHeadCrossAtten(nn.Module):
    def __init__(self, cfg):
        ...
        self.hidden_dim = cfg.MODEL.UISE.HIDDEN_DIM
        self.num_proposals = cfg.MODEL.UISE.NUM_PROPOSALS
        self.conv_kernel_size_1d = cfg.MODEL.UISE.CONV_KERNEL_SIZE_1D
        self.conv_kernel_size_2d = cfg.MODEL.UISE.CONV_KERNEL_SIZE_2D
...
class DySepConvAtten(nn.Module):
    def __init__(self, cfg):
        ...
        self.hidden_dim = cfg.MODEL.UISE.HIDDEN_DIM
        self.num_proposals = cfg.MODEL.UISE.NUM_PROPOSALS
        self.kernel_size = cfg.MODEL.UISE.CONV_KERNEL_SIZE_1D
...
class CrossAttenHead(nn.Module):
    def __init__(self, cfg):
        ...
        self.num_cls_fcs = cfg.MODEL.UISE.NUM_CLS_FCS
        self.num_mask_fcs = cfg.MODEL.UISE.NUM_MASK_FCS
    ...
    def forward(self, features, proposal_kernels, mask_preds, train_flag):
        ...
        if train_flag:
            ###
            cls_feat = cls_feat.reshape(b, t, self.num_proposals, c)
            cls_feat = cls_feat.mean(dim=1, keepdim=True).squeeze(1)
            ##
        ...
        mask_feat = mask_feat.reshape(b, t, self.num_proposals, c)
        mask_feat = mask_feat.mean(dim=1, keepdim=True).squeeze(1)
        mask_kernels = self.fc_mask(mask_feat).squeeze(2)
        # print(mask_kernels.shape, features.shape)
        new_mask_preds = torch.einsum("bqc,btchw->bqthw", mask_kernels, features)
        ###
        ...
        return cls_score, new_mask_preds.permute(0, 2, 1, 3, 4).reshape(-1, self.num_proposals, h, w), obj_feat.permute(0, 1, 3, 2).reshape(B, self.num_proposals, self.hidden_dim, self.conv_kernel_size_2d, self.conv_kernel_size_2d)


class UISEHead(nn.Module):
    def __init__(self, cfg, num_stages, criterion):
        super(UISEHead, self).__init__()
        self.num_stages = num_stages
        self.criterion = criterion
        self.temperature = cfg.MODEL.UISE.TEMPERATIRE

        self.kernels = nn.Conv2d(in_channels=cfg.MODEL.UISE.HIDDEN_DIM, out_channels=cfg.MODEL.UISE.NUM_PROPOSALS, kernel_size=1)
    ...
    def forward(self, features, targets):
        ...
        for stage in range(self.num_stages + 1):
            ...
            if targets is not None:
                ...
                pred_masks = mask_preds.reshape(bs, -1, q, h, w).permute(0, 2, 1, 3, 4) #

                preds = {'pred_logits': cls_scores, 'pred_masks': pred_masks}
