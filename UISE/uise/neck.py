...
class LiteDeformConv(nn.Module):
    def __init__(self, cfg, backbone_shape):
        ...
        in_features = cfg.MODEL.UISE.IN_FEATURES
        in_channels = []
        out_channels = [cfg.MODEL.UISE.AGG_DIM]
        ...
        # self.fuse_conv = nn.Conv2d(in_channels=sum(out_channels[1:]), out_channels=out_channels[-5], kernel_size=1, stride=1, padding=0)
...
class UISENeck(nn.Module):
    def __init__(self, cfg, backbone_shape):
        super().__init__()

        self.deconv = LiteDeformConv(cfg=cfg, backbone_shape=backbone_shape)
        self.loc_conv = nn.Conv2d(in_channels=128+2,#backbone_shape[cfg.MODEL.UISE.IN_FEATURES[0]].channels//2 + 2,
                                  out_channels=cfg.MODEL.UISE.HIDDEN_DIM,
                                  kernel_size=1,
                                  stride=1)
